...
locals {
  ...
  # HTTP API invoke URL (no scheme) for CloudFront origin
  apigw_domain    = replace(aws_apigatewayv2_api.main.api_endpoint, "https://", "")
  apigw_origin_id = "APIGW-${aws_apigatewayv2_api.main.id}"
}
...
resource "aws_cloudfront_distribution" "main" {
  ...
  # Same-origin API: browser calls https://<cloudfront>/chat (avoids CORS to execute-api)
  origin {
